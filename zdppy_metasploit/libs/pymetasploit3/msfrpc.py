    ...
    def login(self, user, password):
        ...
        try:
            ...
            if auth['result'] == 'success':
                ...
                token = self.add_perm_token()  # 创建token
                self.token = token  # 得到token
                return True
        except Exception:
            raise MsfAuthError("登录MSF失败，权限校验不通过，请检查用户名或密码")

    def add_perm_token(self):
        """
        添加一个token
        """
        token = str(uuid.uuid4())  # 生成uuid
        self.call(MsfRpcMethod.AuthTokenAdd, [token])  # 请求token
...
class ModuleManager(MsfManager):
    ...
    def execute(self,
        ...
        """
        执行模块
        @param modtype 模块类型，比如：'exploit'
        @param modname 模块名称，比如：'exploits/windows/http/icecast_header'
        """
        return self.rpc.call(MsfRpcMethod.ModuleExecute,
                             [modtype, modname, kwargs])
